...
//  }
  ...
  {
    delay(400); //don't have this line in the actual code (just for testing)
    alt = (double) (bmp.readAltitude(SEALEVELPRESSURE_HPA)); //don't have this line in the actual code (just for testing)
    Serial.println(bmp.readAltitude(SEALEVELPRESSURE_HPA)); //don't have this line in the actual code (just for testing)
    Serial.println(alt);  //don't have this line in the actual code (just for testing)

  }
  delay(1000); //Delay this to when we want the rocket to start reading values
...
delay(300); //figure out what we want this to be. (this will be the time in between each iteration of the loop)
alt = bmp.readAltitude(SEALEVELPRESSURE_HPA); //may have to equal (alt-(altitude of test site))
Serial.println(alt); //don't have this line in the actual code
if(servo_spin == true)
{
    if(alt>=37.1)// || curr_time >= 12)//change this value for test day (probably should be lower than 8509 feet because there will still be upward momentum)
    {
      Serial.println("about to spin"); //don't have this line in the actual code
      delay(4000);  //don't have this line in the actual code
      myServo.attach(10);
      myServo.write(100);
      servo_spin = false;
    }
...
//Accelerometer Readings
  ...
  /* Print out the values */ //NEED TO CHANGE THESE TO SD.WRITE lines instead of print lines
  ...
  File myFile = SD.open("rocket.log", FILE_WRITE); //CHANGE THIS FILE NAME AFTER YOU FIGURE OUT HOW TO CREATE/LOAD A FILE
  if(myFile)
  {
    //Serial.println(dataString1 + " " + dataString2);
    if (SD.exists("rocket.log")) {
      //Serial.println("it works");
    } else {
      //Serial.println("rocket.log doesn't exist.");
    }
  ...
  {
    //Serial.println("didn't work oops");
  }
}